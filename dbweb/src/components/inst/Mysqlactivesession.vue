    ...
    <el-row>
      <el-col :span="24">
        <el-table ref="sessionMultipleTable" :data="sessionTableData" stripe border v-loading="listLoading" style="width: 100%" max-height="500">
          ...
          <el-table-column prop="host" label="连接主机" ></el-table-column>
          <el-table-column prop="db" label="连接数据库"></el-table-column>
          <el-table-column prop="command" label="执行类型"></el-table-column>
          <el-table-column prop="state" label="执行状态"></el-table-column>
          <el-table-column prop="time" label="执行时间"></el-table-column>
          <el-table-column prop="part_info" label="执行sql">
              ...
              <el-popover
                ...
                <div>
                  ...
                  <el-row style="text-align: center">
                    <el-button type="info" v-clipboard:copy="scope.row.info"
                               v-clipboard:success="onCopy"
                               v-clipboard:error="onError">复制
                    </el-button>
                  </el-row>
                </div>
                <el-button slot="reference" type="text" style="margin:auto 0px auto 10px;">{{ scope.row.part_info }}</el-button>
              </el-popover>
            </template>
          </el-table-column>
        </el-table>
          ...
          this.flag = false,
            ...
            this.$http.get('/api/inst/getmlinfo/', {
              params: {
                ...
                this.sessionTableData = res.result.activ_session;
      ...
      killSession(){
        ...
        if(mulSel.length>0){
          ...
          for(var i=0;i<mulSel.length;i++){
            execSql = execSql + 'kill ' + mulSel[i].id + ';' + "<br/>"
          }
